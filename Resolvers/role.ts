...
// Mutations
const createRole = async (_:any, { input }: {input: IRoleInput} , { token }: {token: string}) => {
    try {
        const userToken = verifyToken(token)
        if(typeof userToken === "string") throw new Error("No se ha enviado un token")
...
const updateRole = async (_:any, { id,input }: {id: string, input: IRoleInput}, { token }: {token: string}) => {
    try {
        const userToken = verifyToken(token)
        if(typeof userToken === "string") throw new Error("No se ha enviado un token")  
        verifyAdmin(userToken)
